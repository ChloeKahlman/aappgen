...
with pandas.ExcelFile(roomsfile) as excel:
  ...
  for roomname in excel.sheet_names:
    ...
    rowcounter = 0
    for cur_row in range(tab.shape[0]):
      columncounter = 0
      foundpc = False
      for cur_col in range(tab.shape[1]):
        cell = str(tab.iloc[cur_row, cur_col])
        if (not cell.startswith("PC-")):
          continue
        else:
